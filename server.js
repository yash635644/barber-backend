...
// =============================================
// 🛠️ HELPER: THE IPv4 HUNTER
// =============================================
async function getSafePool() {
    let connectionString = process.env.DATABASE_URL;
    
    console.log("---------------------------------------------");
    console.log("🕵️  STARTING IPv4 HUNT...");
    ...
    try {
        ...
        // 2. FORCE Network DNS Lookup for IPv4 (Bypasses OS/Container defaults)
        // 'resolve4' queries the DNS server directly for A records (IPv4 only)
        ...
        if (addresses && addresses.length > 0) {
            ...
            // 3. Swap the hostname for the IP
            dbUrl.hostname = ip;
            connectionString = dbUrl.toString();
        } else {
            console.warn("⚠️  No IPv4 address found. Trying default connection...");
        }

    } catch (err) {
        console.error("❌ DNS LOOKUP FAILED:", err.message);
        console.log("⚠️  Falling back to standard connection string...");
    }

    // 4. Create Pool with the SAFE String
    const pool = new Pool({
...
// 🚀 MAIN SERVER START
// =============================================
let pool; // Global pool variable

async function startServer() {
    // Initialize Database
    pool = await getSafePool();
    ...
    try {
        ...
        console.log("👉 HINT: Is the password correct in Render 'Environment'?");
    }
    console.log("---------------------------------------------");
    ...
    app.get('/', (req, res) => res.send('<h1>Backend Online & IPv4 Secured 🚀</h1>'));
    ...
    app.get('/api/admin/stats', async (req, res) => {
        try {
            ...
            // Simplified Stats for Stability
            res.json({ data: { today: parseInt(countRes.rows[0].count), pending: parseInt(pendRes.rows[0].count), revenue: 0, history: [] }});
        } catch (err) { res.status(500).json({ error: err.message }); }
    });
    ...
    app.put('/api/admin/bookings/:id', async (req, res) => {
        ...
        try {
            ...
            if(rows[0] && rows[0].type === 'Online') {
                 let msgBody = status === "Confirmed" ? `✅ Booking Confirmed for ${rows[0].date_time}` : `⚠️ Update: ${status}`;
                 await sendWhatsApp(rows[0].customer_phone, msgBody);
            }
    ...
    app.put('/api/admin/bookings/:id/update', async (req, res) => {
        ...
        try {
            ...
            const { rows } = await pool.query("SELECT * FROM bookings WHERE id = $1", [req.params.id]);
            if(rows[0] && rows[0].type === 'Online') await sendWhatsApp(rows[0].customer_phone, `📅 Updated: ${dateTime}`);
            res.json({ message: "Updated" });
        } catch (err) { res.status(500).json({ error: err.message }); }
    });
